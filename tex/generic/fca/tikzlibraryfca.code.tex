...
\newcommand\fca@tikz@labelBox@label[1]{%
  \typeout{fca@tikz@labelBox@label: |attributes\space \fca@label@position|}%
  \fca@labelslayer
  \path[/fca/.cd,every node/.try,every label/.try,#1]
  (\fca@temp@node@name.\fca@label@at)
  ++(\fca@label@shift@x,\fca@label@shift@y)
  node[/fca/.cd,every node/.try,/tikz/.cd,shape=rectangle,/fca/.cd,every label/.try,#1] (%
...
\newcommand\fca@tikz@labelBox@connector[1]{%
  \typeout{fca@tikz@labelBox@connector}%
  \begin{scope}
    \fca@connectorslayer
...
\newcommand\fca@tikz@labelBox@concept[1]{%
  \typeout{fca@tikz@labelBox@concept}%
...
\newcommand\fca@tikz@labelBox[1]{%
  \begin{scope}%
    \fcaset{%
      draw/.forward to=/tikz/draw,
      every node/.try,every label/.try,#1}%
    \edef\@tempa{\pgfkeysvalueof{/fca/pgfnode}}%
    \xdef\fca@temp@node@name{%
      \expandafter\fca@remove@anchor\expandafter{\@tempa}%
    }%
    \fca@tikz@labelBox@label{#1}%
    \iffca@connectors
      \fca@tikz@labelBox@connector{#1}%
    \fi
    \begin{scope}
      ...
      \ifx\@tempa\tikz@shape
      \else
        \fca@tikz@labelBox@concept{#1}%
      \fi
    \end{scope}
  \end{scope}%
  \typeout{end fca@tikz@labelBox}%
  \ignorespaces
}
